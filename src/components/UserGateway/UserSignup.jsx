import React, { useState } from 'react';
import "../../styles/layout/UserGateway/UserSignup.css"; 

function UserSignup() {
  const [accountType, setAccountType] = useState('personal');

  return (
    <div>
      <div>
        <a href="/">
          <img
            src="data:image/png;base64,"
            alt=""
            className="ms-1 mt-5"
            style={{ height: '58px', width: 'auto' }}
          />
        </a>

        <div className="container-fluid">
          <div className="row full-height-row g-0">
            <div className="col-md-6 d-none d-md-block mt-5">
              <img
                src="https://images.unsplash.com/photo-1508780709619-79562169bc64?auto=format&fit=crop&w=800&q=80"
                alt="Side"
                className="left-image"
              />
            </div>

            <div className="col-md-5 col-12 ">
              <div className="right-content">
                <h3>Create an account</h3>

                {/* Toggle Buttons */}
                <div className="btn-group mb-5" role="group">
                  <input
                    type="radio"
                    className="btn-check"
                    name="accountType"
                    id="personalBtn"
                    autoComplete="off"
                    checked={accountType === 'personal'}
                    onChange={() => setAccountType('personal')}
                  />
                  <label className="btn btn-outline-secondary" htmlFor="personalBtn">Personal</label>

                  <input
                    type="radio"
                    className="btn-check"
                    name="accountType"
                    id="businessBtn"
                    autoComplete="off"
                    checked={accountType === 'business'}
                    onChange={() => setAccountType('business')}
                  />
                  <label className="btn btn-outline-secondary" htmlFor="businessBtn">Business</label>
                </div>

                {/* Personal Form */}
                {accountType === 'personal' && (
                  <form>
                    <div className="row mb-3">
                      <div className="col">
                        <input type="text" className="form-control" placeholder="First Name" required />
                      </div>
                      <div className="col">
                        <input type="text" className="form-control" placeholder="Last Name" required />
                      </div>
                    </div>
                    <div className="mb-3">
                      <input type="email" className="form-control" placeholder="Email" required />
                    </div>
                    <div className="mb-3">
                      <input type="password" className="form-control" placeholder="Password" required />
                    </div>
                    <span className="password-guide">
                      By selecting Create personal account, you agree to our User Agreement and acknowledge reading our User Privacy Notice.
                    </span>
                    <div className="signInButton mt-4">
                      <button type="submit" className="button w-100">Create personal account</button>
                    </div>
                  </form>
                )}

                {/* Business Form */}
                {accountType === 'business' && (
                  <form>
                    <div className="mb-3">
                      <input type="text" className="form-control" placeholder="Business Name" required />
                    </div>
                    <div className="mb-3">
                      <input type="email" className="form-control" placeholder="Business Email" required />
                    </div>
                    <div className="mb-3">
                      <input type="password" className="form-control" placeholder="Password" required />
                    </div>
                    <div className="mb-3">
                      <input type="text" className="form-control" placeholder="Where is your business registered?" required />
                    </div>
                    <span className="password-guide">
                      By selecting Create business account, you agree to our Business Agreement and acknowledge reading our Business Privacy Notice.
                    </span>
                    <div className="signInButton mt-4">
                      <button type="submit" className="button w-100">Create business account</button>
                    </div>
                  </form>
                )}

                {/* Divider */}
                <section style={{ display: 'flex', alignItems: 'center', width: '100%', margin: '20px 10px' }}>
                  <div style={{ flex: 1, borderTop: '1px solid #ccc' }}></div>
                  <span style={{ margin: '0 12px', whiteSpace: 'nowrap' }}>or continue with</span>
                  <div style={{ flex: 1, borderTop: '1px solid #ccc' }}></div>
                </section>

                {/* Social Buttons */}
                <div className="socialButtons" style={{ display: 'flex', gap: '16px' }}>
                  <button aria-label="Continue with Google" style={socialButtonStyle}>
                    <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google logo" style={iconStyle} />
                    Google
                  </button>
                  <button aria-label="Continue with Facebook" style={socialButtonStyle}>
                    <img src="https://upload.wikimedia.org/wikipedia/commons/5/51/Facebook_f_logo_%282019%29.svg" alt="Facebook logo" style={iconStyle} />
                    Facebook
                  </button>
                  <button aria-label="Continue with Apple" style={socialButtonStyle}>
                    <img src="https://www.svgrepo.com/show/303110/apple-black-logo.svg" alt="Apple logo" style={iconStyle} />
                    Apple
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>  
    </div>
  );
}

const socialButtonStyle = {
  backgroundColor: '#fff',
  border: '1px solid #ccc',
  padding: '10px 24px',
  borderRadius: '500px',
  fontWeight: 'bold',
  display: 'flex',
  alignItems: 'center',
  gap: '10px',
  fontSize: '16px',
  cursor: 'pointer',
  minHeight: '48px',
};

const iconStyle = {
  height: '24px',
  width: '24px',
};

export default UserSignup;
